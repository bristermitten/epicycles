...
class (Variables (Var sem)) => LiftVariables (sem :: [k] -> k -> Type) where
    type Var sem :: [k] -> k -> Type
    type Var sem = sem

    type LiftConstraint sem (a :: k) :: Constraint
    type LiftConstraint sem a = ()

    liftVar :: (LiftConstraint sem a) => Var sem env a -> sem env a
    default liftVar :: (Var sem ~ sem, LiftConstraint sem a) => Var sem env a -> sem env a
    liftVar = id

instance LiftVariables Ix
...
-}
runOpen :: (LiftVariables sem, LiftConstraint sem a) => (EnvI sem a -> EnvI sem b) -> sem '[a] b
runOpen = runOpen'

-- | Same vibe as runOpen just with N free variables, represented as type env
runOpenN :: (LiftVariables sem) => OfLength sem as -> (Env (EnvI sem) as -> EnvI sem a) -> sem as a
runOpenN = runOpenN'
...
runOpenV ::
    forall sem n a b.
    (LiftVariables sem, LiftConstraint sem a) => SNat n -> (Vec (EnvI sem a) n -> EnvI sem b) -> sem (Repeat a n) b
runOpenV sn f = runOpenN (mkOfLength sn) (f . e2v sn)
  where
    mkOfLength :: forall m. SNat m -> OfLength sem (Repeat a m)
    mkOfLength SZ = LZ
    mkOfLength (SS n) = LS (mkOfLength n)
...
liftFO ::
    (forall env. Env (sem env) as -> sem env a) -> -- generic handling of a semantic function
...
data URep (sem :: [k] -> k -> Type) (s :: Sig2 k) where
    UR :: OfLength sem as -> (Env (EnvI sem) as -> EnvI sem b) -> URep sem (as ':~> b)
...
liftSO f ks = EnvI $ \e -> f (mapEnv (conv e) ks)
  where
    conv :: TEnv env -> URep sem s -> TermRep sem env s
    conv e (UR e1 k) = TR $ convertConstructArg e e1 k

-- Here down is the type families (leave in appendix)

data OfLength (sem :: [k] -> k -> Type) (as :: [k]) where
    LZ :: OfLength sem '[]
    LS :: (LiftConstraint sem a) => !(OfLength sem as) -> OfLength sem (a ': as)

-- | Class to reuse 'ol1', ..., 'ol4' for 'liftSOn', 'liftSOn'', and 'liftSOnGen'.
class LiftOfLength sem f as t | t -> as where
    liftOfLength :: OfLength sem as -> f t

instance LiftOfLength sem (OfLength sem) as as where
    liftOfLength = id

-- handy short cuts for values:

-- | Smart constructor for an 'OfLength' zero.
ol0 :: forall sem f t. (LiftOfLength sem f '[] t) => f t
ol0 = liftOfLength (LZ :: OfLength sem '[])

-- | Smart constructor for an 'OfLength' one.
ol1 :: forall sem f a t. (LiftOfLength sem f '[a] t, LiftConstraint sem a) => f t
ol1 = liftOfLength (LS (LZ :: OfLength sem '[]))

-- | Smart constructor for an 'OfLength' two.
ol2 :: forall sem f a b t. (LiftOfLength sem f '[a, b] t, LiftConstraint sem a, LiftConstraint sem b) => f t
ol2 = liftOfLength (LS (LS (LZ :: OfLength sem '[])))

-- | Smart constructor for an 'OfLength' three.
ol3 :: forall sem f a b c t. (LiftOfLength sem f '[a, b, c] t, LiftConstraint sem a, LiftConstraint sem b, LiftConstraint sem c) => f t
ol3 = liftOfLength (LS (LS (LS (LZ :: OfLength sem '[]))))

-- | Smart constructor for an 'OfLength' four.
ol4 :: forall sem f a b c d t. (LiftOfLength sem f '[a, b, c, d] t, LiftConstraint sem a, LiftConstraint sem b, LiftConstraint sem c, LiftConstraint sem d) => f t
ol4 = liftOfLength (LS (LS (LS (LS (LZ :: OfLength sem '[])))))
...
data DimSimple (sem :: [k] -> k -> Type) (s :: Sig2 k) where
    DimSimple :: !(OfLength sem as) -> DimSimple sem (as ':~> a)

instance (t ~ (as ':~> a)) => LiftOfLength sem (DimSimple sem) as t where
    liftOfLength = DimSimple

fromFuncTerm ::
    FuncTerm sem env ss r ->
    Env (TermRep sem env) ss ->
    sem env r
...
type family
    ...
    FuncU sem ((as ':~> a) ': ss) r =
        Func (EnvI sem) as (EnvI sem a) ->
        FuncU sem ss r

toFuncU :: Dim sem ss -> (Env (URep sem) ss -> EnvI sem r) -> FuncU sem ss r
toFuncU ENil f = f ENil
toFuncU (ECons (DimSimple n) ns) f = \k -> toFuncU ns (f . ECons (toURep n k))

toURep :: OfLength sem as -> Func (EnvI sem) as (EnvI sem r) -> URep sem (as ':~> r)
toURep n f = UR n (fromFunc f)

ofl2TEnv :: OfLength sem as -> TEnv as
ofl2TEnv LZ = ENil
ofl2TEnv (LS n) = ECons Proxy (ofl2TEnv n)

type Dim sem = Env (DimSimple sem)
...
liftSOn ::
    ...
    (LiftVariables sem) =>
    Dim sem ss ->
    (forall env. FuncTerm sem env ss r) ->
    FuncU sem ss r
liftSOn ns f =
...
data HRep' (semExp :: [k] -> k -> Type) (s :: SemSig k) where
    HR' :: OfLength semExp as -> (Env (EnvI semExp) as -> EnvI sem b) -> HRep' semExp (MkSemSig sem as b)

convertConstructArg :: (LiftVariables semExp) => TEnv env -> OfLength semExp as -> (Env (EnvI semExp) as -> EnvI sem b) -> sem (Append as env) b
convertConstructArg shEnv shAs k =
    let shAsEnv = appendEnv (ofl2TEnv shAs) shEnv -- TEnv (as ++ env)
        xs = makeVariables shEnv shAs shAsEnv -- Env (EnvI semExp) as
     in runEnvI (k xs) shAsEnv

makeVariables :: forall semExp proxy env as. (LiftVariables semExp) => proxy env -> OfLength semExp as -> TEnv (Append as env) -> Env (EnvI semExp) as
makeVariables _ LZ _ = ENil
makeVariables p (LS (shAs :: OfLength semExp as')) te@(ECons _ te') =
...
liftSOF' ::
    ...
    (forall semsig. h semsig -> HRep' semE semsig) ->
    EnvI sem r
liftSOF' f k = EnvI $ \shEnv -> f (convertHRep'toSemRep' shEnv . k)

toHRep' :: OfLength semExp as -> Func (EnvI semExp) as (EnvI sem r) -> HRep' semExp (MkSemSig sem as r)
toHRep' n f = HR' n (fromFunc f)
...
toFuncH' :: Dim' semExp ss -> (Env (HRep' semExp) ss -> EnvI semR r) -> FuncH' semExp semR ss r
...
data DimMult (sem :: [k] -> k -> Type) (s :: SemSig k) where
    DimMult :: !(OfLength sem as) -> DimMult sem (MkSemSig semR as a)

instance (t ~ MkSemSig semR as a) => LiftOfLength sem (DimMult sem) as t where
    liftOfLength = DimMult

type Dim' sem = Env (DimMult sem)
...
liftFO' ::
    (forall env. Env (SemRepFO env) ss -> sem env a) ->
    Env HRepFO ss ->
    EnvI sem a
...
runOpen' :: (LiftVariables semE, LiftConstraint semE a) => (EnvI semE a -> EnvI sem b) -> sem '[a] b
runOpen' f = runOpenN' (LS LZ) (\(ECons x _) -> f x)

-- | Same vibe as 'runOpen'' just with N free variables, represented as type env
runOpenN' :: (LiftVariables semE) => OfLength semE as -> (Env (EnvI semE) as -> EnvI sem a) -> sem as a
runOpenN' n f =
    ...
    let e = ofl2TEnv n -- make arg
        xs = mkXs e n
     in runEnvI (f xs) e -- apply f, unpack result
  where
    -- make env of terms using type env
    mkXs :: forall sem as'. (LiftVariables sem) => TEnv as' -> OfLength sem as' -> Env (EnvI sem) as'
    mkXs ENil LZ = ENil
    mkXs te@(ECons _ te') (LS n') =
        let x = EnvI $ \e' -> liftVar $ weakenMany te e' var
         in ECons x (mkXs te' n')
...
data ArgSpec k
    = forall k'. MkArgSpec
        -- | The semantic domain for the argument to be interpreted.
        ([k] -> k' -> Type)
        -- | The binder spec of the argument
        (BindSpec k)
        -- | The (guest) type of the argument
        k'
...
data HRepGen (semExp :: [k] -> k -> Type) (aspec :: ArgSpec k) where
    HRGen ::
        OfLength semExp as ->
        (Env (EnvI semExp) as -> Env HRepK bs -> EnvI sem b) ->
        HRepGen semExp (MkArgSpec sem (MkBindSpec as bs) b)
...
convertConstructArgGen :: (LiftVariables semE) => TEnv env -> OfLength semE as -> (Env (EnvI semE) as -> Env HRepK bs -> EnvI sem b) -> Env (SemRepK (Append as env)) bs -> sem (Append as env) b
convertConstructArgGen shEnv shAs k kargs =
    let shAsEnv = appendEnv (ofl2TEnv shAs) shEnv -- TEnv (Append as env)
        xs = makeVariables shEnv shAs shAsEnv -- Env (EnvI semExp) as
...
liftSOGen ::
    ...
    Env (HRepGen semExp) ss ->
    EnvI sem r
liftSOGen f ks = EnvI $ \shEnv -> f (mapEnv (convertHtoSemGen shEnv) ks)

-- | Binding description.
data BDesc (semExp :: [k] -> k -> Type) (as :: [k]) (bs :: [KBindSpec k]) where
    E :: BDesc semExp '[] '[]
    -- | stands for the corresponding binding is to be kept (for further processing afterwards).
    K :: (Weakenable sem) => !(BDesc semExp as bs) -> BDesc semExp as (MkKBindSpec sem b : bs)
    -- | stands for the corresponding binding is to be unembedded.
    U :: (LiftConstraint semExp a) => !(BDesc semExp as bs) -> BDesc semExp (a : as) bs

descToOfLength :: BDesc semExp as bs -> OfLength semExp as
descToOfLength E = LZ
descToOfLength (K d) = descToOfLength d
descToOfLength (U d) = LS (descToOfLength d)
...
fromBsFunc :: BDesc semExp as bs -> BsFunc bs semR r -> Env HRepK bs -> EnvI semR r
fromBsFunc E f _ = f
fromBsFunc (K d) f (ECons (HRK e) es) = fromBsFunc d (f e) es
fromBsFunc (U d) f es = fromBsFunc d f es

toHRepGen :: BDesc semExp as bs -> Func (EnvI semExp) as (BsFunc bs semR r) -> HRepGen semExp (MkArgSpec semR (MkBindSpec as bs) r)
toHRepGen d f = HRGen (descToOfLength d) (fromBsFunc d . fromFunc f)

data DimNested (sem :: [k] -> k -> Type) (s :: ArgSpec k) where
    DimNested :: !(BDesc sem as bs) -> DimNested sem (MkArgSpec semR (MkBindSpec as bs) a)

instance (t ~ MkArgSpec semR (MkBindSpec as '[]) a) => LiftOfLength sem (DimNested sem) as t where
    liftOfLength = DimNested . go
      where
        go :: OfLength sem xs -> BDesc sem xs '[]
        go LZ = E
        go (LS x) = U (go x)

type DimGen sem = Env (DimNested sem)
...
toFuncHGen :: DimGen semExp ss -> (Env (HRepGen semExp) ss -> EnvI semR r) -> FuncHGen semExp semR ss r
...
fromFuncSemK ::
    BDesc semExp as0 bs -> (Env (SemRepK asenv) bs -> sem asenv b) -> BsFuncSem bs asenv (sem asenv b)
fromFuncSemK E f = f ENil
fromFuncSemK (U d) f = fromFuncSemK d f
fromFuncSemK (K d) f = \s -> fromFuncSemK d (f . ECons (SemRK s))

fromFuncSemGen :: DimGen semExp ss -> FuncSemGen semR env ss r -> Env (SemRepGen env) ss -> semR env r
