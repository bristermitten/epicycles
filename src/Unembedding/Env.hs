...
deriving instance (forall x. Show (f x)) => Show (Env_ f as)

instance (forall x. Show (f x)) => Show (Env f as) where
    showsPrec k (MkEnv _ es) = showsPrec k es
...
ixToInt :: Ix env a -> Int
ixToInt ix = fromIntegral Int (ixToWord ix)
...
setIx :: Ix as a -> f a -> Env f as -> Env f as
setIx IxZ v (ECons _ rest) = ECons v rest
setIx (IxS n) v (ECons x rest) = ECons x (setIx n v rest)
